...
const ItemsContainer = () => {
    ...
    const renderItems = (arr) => arr.map((el, i) => <Item key={i}
                                                    ...
                                                    btnPathname={el.source}
                                                    morePhotos={el.morePhotos}
                                                    soldOut={el.soldOut} />)
    const allItems = telurian2.concat(items)
    return(
        <div className='items-container'>
            {pathname === '/about-telurian-1' ? <p>Three Different High-end Versions</p> : pathname === '/about-telurian-2' ? <p>Telurian 2 is available in two versions</p> : null} 
